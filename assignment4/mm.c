...
int mm_init() {
    ...
    fprintf(stderr, "mm_init is called\n");
    ...
    // Reserve the first word for heap header (as described in the next comment)
    // The first block will start at the 2nd word
    ...
    // requestMoreSpace() for more info, but you could ignore it).
    ...
    // we will set TAG_PRECEDING_USED to indicate that its preceding block in memory is used
    // That avoids the coalesce with its preceding block.
    firstFreeBlock->sizeAndTags = totalSize | TAG_PRECEDING_USED;
    firstFreeBlock->next = NULL;
    firstFreeBlock->prev = NULL;
    // Set the boundary tag of firstFreeBlock
...
void *mm_malloc(size_t size) {
    size_t reqSize;                     // Total Size of block including header
    BlockInfo *ptrFreeBlock = NULL;     // Pointer to free block we're working with
    size_t blockSize;                   // Size of allocated block w/o Header
    size_t precedingBlockUseTag;        // Preceding use tag of found block

    if (size == 0) return NULL;         // Zero-size requests get NULL.

    size += WORD_SIZE;                  // Add one word for the initial size header.
    if (size <= MIN_BLOCK_SIZE) {       // If block size is less than or equal to minimum size
        reqSize = MIN_BLOCK_SIZE;       // Increase reqSize to minimum size
    } else {                          // If reqSize is greater than minimum size
        // Round reqSize up to closest multiple of 8 bytes
        reqSize = ALIGNMENT * ((size + ALIGNMENT - 1) / ALIGNMENT);
    }

    if ((ptrFreeBlock = (BlockInfo *) searchFreeList(reqSize)) == NULL) { // Search freeList & check return value
        requestMoreSpace(reqSize);                        // Request more space
        ptrFreeBlock = searchFreeList(reqSize);           // Grab block from freeList
    }
    removeFreeBlock(ptrFreeBlock);                                                  // Remove block from the freeList
    blockSize = SIZE((*ptrFreeBlock).sizeAndTags);                   		    // Get block size with SIZE() macro
    precedingBlockUseTag = (*ptrFreeBlock).sizeAndTags & TAG_PRECEDING_USED;        // Get value of Bit 1

    if (blockSize - reqSize >= MIN_BLOCK_SIZE) { // If block is more than one minimum block size more
        /* Split a large block into two pieces */

        /* Create new header for allocated tag block */
        // Reset the size and used tag for the first block & set precedingBlockUseTag
        (*ptrFreeBlock).sizeAndTags = reqSize | TAG_USED | precedingBlockUseTag;

        /* Set up for the 2nd block */
        BlockInfo *nextBlock = (BlockInfo *) UNSCALED_POINTER_ADD(ptrFreeBlock, reqSize);
        // Save the new size and tag info in the block info of the next block.
        (*nextBlock).sizeAndTags = (blockSize - reqSize) | TAG_PRECEDING_USED;
        // Setting the boundary tag for the 2nd block
        *((size_t *) UNSCALED_POINTER_ADD(ptrFreeBlock, blockSize - WORD_SIZE)) =
                (blockSize - reqSize) | TAG_PRECEDING_USED;

        // Put the new block in the free list using both insertFreeBlock() and coalesceFreeBlock()
        insertFreeBlock(nextBlock);
        coalesceFreeBlock(nextBlock);
    } else { // Now there is no need to split it
        // Update block header to be used and correct size
        (*ptrFreeBlock).sizeAndTags = (*ptrFreeBlock).sizeAndTags | TAG_USED;

        // Update following block's header/footer preceding tag bit
...
void mm_free(void *ptr) {
    ...
    blockInfo = (BlockInfo *) UNSCALED_POINTER_SUB(ptr, WORD_SIZE);        // Get block info
    payloadSize = SIZE((*blockInfo).sizeAndTags);                       // Get block payload
    ...
    *((size_t *) UNSCALED_POINTER_ADD(blockInfo, payloadSize - WORD_SIZE)) = (*blockInfo).sizeAndTags;
    ...
    // update its header (since its preceding block is now free)
    (*followingBlock).sizeAndTags = (*followingBlock).sizeAndTags & (~TAG_PRECEDING_USED);

    insertFreeBlock(blockInfo);                                            // Insert free block into free list
    coalesceFreeBlock(blockInfo);                                          // Coalesce free block
}
